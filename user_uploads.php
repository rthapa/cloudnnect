...
$uid = "";
if(isset($_GET["uid"])){
	$uid = mysqli_real_escape_string($db_conx, $_GET["uid"]);// preg_replace('#[^a-z0-9]#i', '', $_GET['f']);
}else{
	header("location: index.php");
    exit();
}
$isViewerAlsoUser = false;

//checking if the viwer is also the logged user
if($log_id == $uid){
	//header("location: index.php");
    //exit();
    $isViewerAlsoUser = true;
}
?>
<?php
//
...
$ResultList ="";
$keywordList ="";
if($isViewerAlsoUser != true){
	$sql = "SELECT * FROM file WHERE owner_id='$uid' AND type='a' LIMIT 20";
}
$sql = "SELECT * FROM file WHERE owner_id='$uid' LIMIT 20";
	...
	<body>
		...
		<div class="searchWrapper">
			<div class="searchContent">
				...
				<h5>(<?php echo $queryNumRows;?>) Total Files</h5>
				</div>
				<!--list of files here -->
				<?php echo $ResultList; ?>
			</div>
